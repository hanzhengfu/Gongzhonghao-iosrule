   ...
   try:
      ...
      for ac in range(1):
        for k in range(0,1):
          ...
          Av(urllist[len(urllist)-1],hd,(len(urllist)))
          #print(alllist)
          liveId(urllist[len(urllist)-4])
          if len(alllist)>10:
            allinbd(alllist)
          print('【C】'+str(k+1))
          ...
          print('await.............' )
          time.sleep(random.randint(30,50)/len(hdlist))
          Av(urllist[len(urllist)-3],hd,(3),json.dumps(body3))
          Av(urllist[len(urllist)-2],hd,(4),json.dumps(body4))
          time.sleep(random.randint(20,30)/len(hdlist))
        print('<<<<<<<'+str(ac+1)+'>>>>>>>>>')
